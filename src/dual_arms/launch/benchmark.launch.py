...
def generate_launch_description():
    ...
    # GAZEBO: world file
    robot_gazebo = "/home/marwan/full_system/src/worlds/empty_minimal.world"
    ...
    kinematics_path = "/home/marwan/full_system/install/dual_arms/share/dual_arms/config/kinematics.yaml"
    ...
    # --- START BENCHMARKING CONFIGURATION ADDITION ---
    
    # Load additional config files for benchmarking
    benchmark_full_config = load_yaml(package_name, "config/benchmark.yaml")
    
    # 1. Separate OMPL/TrajOpt configurations for the benchmark node's specific planner parameters
    # Copy the full OMPL config, then pop the pipeline section (leaving the planner definitions)
    ompl_planning_for_benchmark = ompl_planning.copy()
    ompl_planning_pipeline_config_raw = ompl_planning_for_benchmark.pop('planning_pipelines', {})
    

    # 2. Combine all individual planner parameters (RRTConnectkConfigDefault, TrajOptDefault, etc.)
    planner_parameters_from_benchmark = benchmark_full_config.get('planner_parameters', {})
    
    combined_planner_parameters = {}
    combined_planner_parameters.update(ompl_planning_for_benchmark)
    combined_planner_parameters.update(planner_parameters_from_benchmark)
    
    # 3. Create the pipeline configurations (for the benchmark node and RViz)
    # This is the correct, un-nested structure required by the PlanningFrame (RViz) and Benchmark Node
    ompl_pipeline_config_for_benchmark_rviz = {"planning_pipelines": ["ompl"]}
    if 'ompl' in ompl_planning_pipeline_config_raw:
        ompl_pipeline_config_for_benchmark_rviz['ompl'] = ompl_planning_pipeline_config_raw['ompl']
    

    # 4. Define the benchmark node parameters
    benchmark_params = {
        "benchmark_config": os.path.join(robot_description_path, 'config', 'benchmark.yaml'),
        "output_directory": os.path.join(os.environ['HOME'], 'full_system', 'benchmark_results'),
        "use_sim_time": True,
    }
    
    # 5. Define the benchmark node
    run_benchmark_node = Node(
        ...
        parameters=[
            ...
            joint_limits,
            combined_planner_parameters,
            ompl_pipeline_config_for_benchmark_rviz, 
            trajectory_execution,
            moveit_controllers,
            planning_scene_monitor_parameters,
            benchmark_params,
    ...
    # --- ADDED BENCHMARKING TIMER ---
    LD.add_action(TimerAction(
        period=10.0,
        actions=[run_benchmark_node]
    ))
