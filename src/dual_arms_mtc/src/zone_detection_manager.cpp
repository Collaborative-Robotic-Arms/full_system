...
void ZoneDetectionManager::on_diagnostic_timer() {
    ...
    try {
        ...
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "Zone TF Error: %s", ex.what());
        return;
    }

    // 2. CHECK FOR COLLISION RISK & PRINT DISTANCE
    if (poses_valid) {
        // Calculate and print the live distance
        double separation = calculate_distance(ar4_pose, abb_pose);
        RCLCPP_INFO(get_logger(), "Live Arm Separation: %.3f meters", separation);

        if (check_collision_risk(ar4_pose, abb_pose)) {
            publish_zone_status("COLLISION_WARNING");
            return; // Halt normal output
        }
    }

    // 3. Convert Zone ENUMS to string
