...
class AssemblySupervisor(Node):
    ...
    def transform_pose_to_abb(self, input_pose):
        """
        Manually transforms Camera Frame -> Base Frame using user-provided values.
        Translation: [-0.010, -0.670, 0.803]
        Rotation: [0.707, 0.707, 0.000, -0.000]
        """
        try:
            # Create a hardcoded Transform object instead of looking it up
            t = TransformStamped()
            
            # --- TRANSLATION ---
            t.transform.translation.x = 0.67
            t.transform.translation.y = 0.01
            t.transform.translation.z = 0.804
            
            # --- ROTATION ---
            t.transform.rotation.x = 0.707
            t.transform.rotation.y = 0.707
            t.transform.rotation.z = 0.000
            t.transform.rotation.w = 0.000

            # Apply the transform
            transformed_pose = tf2_geometry_msgs.do_transform_pose(input_pose, t)
            
            self.get_logger().info(f"Hardcoded Transform applied: World Z became {transformed_pose.position.z:.3f}")
    ...
    async def run_ar4_mission(self, brick):
        ...
        try:
            ...
            if grasp_result_ar and grasp_result_ar.success:
                ...
                grasp_pose_ar.position.z = 0.125
                ...
                if pick_success_ar:
                    ...
                    # Height Adjustment
                    place_goal_ar.target_pose.position.z = 0.145
